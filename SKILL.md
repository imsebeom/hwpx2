...
- 이미지: `treatAsChar="1"` (글자처럼 취급)
- **표: `treatAsChar="0"`** — `"1"`로 두면 표가 한 문단에 갇혀 쪽을 넘기지 못하고 **넘친 행이 통째로 사라진다**(실측: 121행 표에서 63행 소실, 문서는 정상적으로 열려 눈치채기 어렵다). 짧은 표는 0과 1의 렌더 결과가 픽셀 단위로 같으므로 언제나 0이 안전하다
...
> ⚠ **그 경로(`_set_cell_multi_paragraph`)는 셀의 기존 문단을 전부 제거하고 새로 만든다** — 라벨
> **이미지가 든 셀**에 줄바꿈 텍스트를 넣으면 그 이미지가 통째로 사라진다(양식지의 배너·아이콘이 이에 해당).
> 채우기 전에 `tc.iter()`로 `binaryItemIDRef` 유무를 보고, 있으면 마지막 문단을 템플릿 삼아 **새 문단을 append**하라.
> 줄바꿈 없는 단일 텍스트는 첫 `<hp:t>`만 갈아 끼우므로 안전하다. (2026-08-25 교육력제고 보고서 양식 실측)
